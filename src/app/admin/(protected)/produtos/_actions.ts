...
const productSchema = z.object({
  ...
  // NCM opcional: vazio = usa o padrão da categoria (ver lib/fiscal.ts).
  ncm: z.string().max(10).optional(),
  // Medidas da peça JÁ EMBALADA, usadas na cotação e na compra da etiqueta.
  // Zero = usa o pacote padrão de Admin → Entrega e frete. Os máximos são os
  // limites das transportadoras: acima disso o Melhor Envio recusa.
  weightGrams: z.coerce.number().int().min(0).max(30000).optional(),
  lengthCm: z.coerce.number().int().min(0).max(105).optional(),
  widthCm: z.coerce.number().int().min(0).max(105).optional(),
  heightCm: z.coerce.number().int().min(0).max(105).optional(),
