...
import { customerSignIn, customerSignUp, type AuthState } from "@/lib/customer/actions";
...
export function CustomerAuth() {
  ...
  const state = mode === "login" ? inState : upState;
    ...
    <div className="mx-auto w-full max-w-sm">
      ...
      {mode === "login" ? (
        <form action={inAction} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={inPending} className="mt-2 border border-content py-3 text-[13px] uppercase tracking-[0.2em] transition-colors hover:bg-content hover:text-surface disabled:opacity-50">
        ...
        <form action={upAction} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={upPending} className="mt-2 border border-content py-3 text-[13px] uppercase tracking-[0.2em] transition-colors hover:bg-content hover:text-surface disabled:opacity-50">
            {upPending ? "Criando..." : "Criar conta"}
          </button>
        </form>
      )}

      {state.error && <p className="mt-4 text-center text-[13px] text-content/70">{state.error}</p>}
      {state.info && <p className="mt-4 text-center text-[13px] text-content/60">{state.info}</p>}
    </div>
  );
}
